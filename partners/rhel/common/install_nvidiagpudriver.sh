...
echo 'export PATH="${PATH:+$PATH:}/usr/local/cuda/bin"' | tee -a /etc/bash.bashrc
echo 'export LD_LIBRARY_PATH="${LD_LIBRARY_PATH:+$LD_LIBRARY_PATH:}/usr/local/cuda/lib64"' | tee -a /etc/bash.bashrc
...
$RHEL_COMMON_DIR/install_nvidia_fabric_manager.sh
